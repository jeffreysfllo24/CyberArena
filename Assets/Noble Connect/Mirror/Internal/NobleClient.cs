...
namespace NobleConnect.Mirror
{
    ...
    public class NobleClient
    {
        ...
        public NobleClient(GeographicRegion region = GeographicRegion.AUTO, Action<string> onFatalError = null, int relayLifetime = 60, int relayRefreshTime = 30, float allocationResendTimeout = .1f, int maxAllocationResends = 8, float requestTimeout = .2f)
        {
        ...
        /// <param name="allocationResendTimeout">Initial timeout before resending refresh messages. This is doubled for each failed resend.</param>
        /// <param name="maxAllocationResends">Max number of times to try and resend refresh messages before giving up and shutting down the relay connection. If refresh messages fail for 30 seconds the relay connection will be closed remotely regardless of these settings.</param>
        public NobleClient(string relayServerAddress, Action<string> onFatalError = null, int relayLifetime = 60, int relayRefreshTime = 30, float allocationResendTimeout = .1f, int maxAllocationResends = 8, float requestTimeout = .2f)
        {
            var settings = (NobleConnectSettings)Resources.Load("NobleConnectSettings", typeof(NobleConnectSettings));
        ...
        public void SetConnectPort(ushort port)
        {
            ...
            {
                hasUDP = true;
                var ignorance = (kcp2k.KcpTransport)Transport.activeTransport;
                ignorance.Port = port;
            }
            if (!hasUDP)
            {
        ...
        /// </summary>
        public bool isHostClient => NetworkClient.isHostClient;
