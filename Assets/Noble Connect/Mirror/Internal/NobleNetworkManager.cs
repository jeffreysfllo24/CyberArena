...
namespace NobleConnect.Mirror
{
    public class NobleNetworkManager : NetworkManager
    {
        ...
        UnityAction onClientAuthenticated;
        ...
        /// <summary>We need a reference to Mirror's internal connect method but it is not public so we get it via reflection.</summary>
        /// <remarks>
        /// This is used so that we can replace Mirror's Connect message handler with our own in NetworkServer, 
        /// but still call Mirror's internal connect method.
        /// </remarks>
        ...
        override public void Awake()
        {
            ...
            var onServerConnectInternalMethod = typeof(NetworkManager).GetMethod("OnServerConnectInternal", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            onClientConnectInternal = (Action)Delegate.CreateDelegate(typeof(Action), this, onClientConnectInternalMethod);
            onClientDisconnectInternal = (Action)Delegate.CreateDelegate(typeof(Action), this, onClientDisconnectInternalMethod);
            onServerConnectInternal = (Action<NetworkConnectionToClient>)Delegate.CreateDelegate(typeof(Action<NetworkConnectionToClient>), this, onServerConnectInternalMethod);
            var onClientAuthenticatedAction = (Action)Delegate.CreateDelegate(typeof(Action), this, onClientAuthenticatedMethod);
            onClientAuthenticated = new UnityAction(onClientAuthenticatedAction);
        ...
        {
            client = new NobleClient(region, OnFatalError, relayLifetime, relayRefreshTime, relayRefreshTimeout, maxRelayRefreshAttempts, requestTimeout);
        ...
        public void RegisterClientHandlers()
        {
            ...
            // Mirror's network manager will have set the OnConnectedEvent and OnDisconnected event
            // to internal methods, but we need those events to call the methods they are hooked up
            // to in NobleClient, so we will re-set the events here, and make sure
            // that the Mirror functions are also still called.
        ...
        override public void OnStartServer()
        {
            ...
            if (!isLANOnly)
            {
                try
                {
                    ...
                    NobleServer.requestTimeout = requestTimeout;
            ...
            // This is really dumb, but immediately after this method Mirror is going to RegisterServerHandlers
            // which is going to replace the OnConnectedEvent and OnDisconnectedEvent on NetworkServer
            // which is going to break NobleServer which relies on those events. So...just going to wait
            // a frame to let that method execute and then re-override the events. Oh Mirror...you so silly.
            // Why not make things virtual and public so it don't have to be this way?
            StartCoroutine(FixMirrorEvents());
        }

        IEnumerator FixMirrorEvents()
        {
            yield return null;
            // Mirror's network manager will have set the OnConnectedEvent and OnDisconnected event
            // to internal methods, but we need those events to call the methods they are hooked up
            // to in NobleServer, so we will re-set the events here, and make sure
            // that the Mirror functions are also still called.
            var onConnectedProp = typeof(NetworkServer).GetField("OnConnectedEvent", BindingFlags.Static | BindingFlags.Public);
            onConnectedProp.SetValue(null, new Action<NetworkConnectionToClient>(OnServerConnectInternal));
        }
        ...
        public override void OnClientDisconnect()
        {
            ...
            StartCoroutine(StopClientAfterDelay(NetworkClient.connection));
        }
